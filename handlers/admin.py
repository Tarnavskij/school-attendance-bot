...
async def download_excel(callback: CallbackQuery) -> None:
    ...
    await callback.message.answer_document(
        document,
        caption=f"📅 Сводка за {date.today().strftime('%d.%m.%Y')}",
    )
...
def _teacher_list_keyboard(teachers, current_page: int, total_pages: int) -> InlineKeyboardMarkup:
    ...
    for t in teachers:
        ...
        label = (
            ...
            f"{ROLE_LABELS.get(t.role, t.role)} · "
            f"{t.class_name or 'нет'}{school_part}"
        )
    ...
    if current_page > 1:
        nav_buttons.append(
            InlineKeyboardButton(text="◀️", callback_data=f"admin:teachers_page:{current_page - 1}")
        )
    nav_buttons.append(
...
async def change_role_menu(callback: CallbackQuery) -> None:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        *[
            [InlineKeyboardButton(
                text=ROLE_LABELS[r],
                callback_data=f"admin:setrole:{teacher_id}:{r}",
            )]
            for r in ALL_ROLES
        ],
        [InlineKeyboardButton(text="❌ Отмена", callback_data=f"admin:teacher:{teacher_id}")],
...
async def set_class_menu(callback: CallbackQuery) -> None:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        *[
            [InlineKeyboardButton(
                text=c.name,
                callback_data=f"admin:assignclass:{teacher_id}:{c.id}",
            )]
            for c in classes
        ],
        [InlineKeyboardButton(text="❌ Отмена", callback_data=f"admin:teacher:{teacher_id}")],
...
async def _show_student_page(
    ...
    rows = [
        [InlineKeyboardButton(text=f"🗑 {s.name}", callback_data=f"admin:delstudent:{s.id}:{class_id}")]
        for s in students
    ]
    ...
    if page > 1:
        nav_buttons.append(
            InlineKeyboardButton(text="◀️", callback_data=f"admin:students_page:{class_id}:{page - 1}")
        )
...
async def list_schools(message: Message) -> None:
    ...
    current_id = get_school_id_for_admin(message.from_user.id)
    current_name = next((s["name"] for s in schools if s["id"] == current_id), "неизвестно")
...
async def restore_admin(message: Message) -> None:
    ...
    from database import Teacher as TeacherModel
    db = SessionLocal()
    try:
        teacher = db.query(TeacherModel).filter(
            TeacherModel.telegram_id == ADMIN_TELEGRAM_ID
        ).first()
        if teacher:
