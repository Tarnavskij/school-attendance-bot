...
from database import (
    SessionLocal, Teacher, Class, Student,
    AttendanceSession, AttendanceRecord, RegistrationRequest, School,
)
...
def _teacher_to_dto(t: Teacher) -> TeacherDTO:
    """Конвертирует ORM-объект Teacher в DTO (без обращения к сессии)."""
    return TeacherDTO(
        id=t.id,
...
def get_teacher_by_telegram_id(telegram_id: int) -> TeacherDTO | None:
    """
    Возвращает активного учителя.
    Для администраторов ищет в текущей активной школе контекста;
    для обычных пользователей — в любой школе (у них ровно одна).
    """
    ...
    with get_db() as db:
        query = (
            db.query(Teacher)
            .options(joinedload(Teacher.class_), joinedload(Teacher.school))
            .filter(Teacher.telegram_id == telegram_id, Teacher.is_active == True)
        )
        # Администратора ищем строго в текущей школе контекста
        if telegram_id == ADMIN_TELEGRAM_ID:
            query = query.filter(Teacher.school_id == school_id)
        else:
            # Для обычного учителя тоже привязываем к school_id контекста,
            # потому что учитель может быть зарегистрирован в нескольких школах.
            query = query.filter(Teacher.school_id == school_id)

        t = query.first()
        return _teacher_to_dto(t) if t else None
...
def get_all_teachers() -> list[TeacherDTO]:
    with get_db() as db:
        teachers = (
            db.query(Teacher)
            .options(joinedload(Teacher.class_), joinedload(Teacher.school))
            .filter(Teacher.school_id == get_current_school_id())
            .all()
        )
        return [_teacher_to_dto(t) for t in teachers]
...
def get_all_classes() -> list[ClassDTO]:
    with get_db() as db:
        return [
            ClassDTO(id=c.id, name=c.name)
            for c in db.query(Class)
            .filter(Class.school_id == get_current_school_id())
            .all()
        ]
...
def get_available_classes(today_date: date, is_admin_user: bool = False) -> list[ClassDTO]:
    ...
    with get_db() as db:
        taken_ids = db.query(AttendanceSession.class_id).filter(
        ...
        return [
            ...
            for c in db.query(Class)
            .filter(
                Class.school_id == get_current_school_id(),
...
def get_students_by_class(class_id: int) -> list[StudentDTO]:
    with get_db() as db:
        return [
            StudentDTO(id=s.id, name=s.name, class_id=s.class_id)
...
def get_session_records(session_id: int) -> list[RecordDTO]:
    with get_db() as db:
        return [
            RecordDTO(student_id=r.student_id, is_present=r.is_present, reason=r.reason)
...
def get_absent_students_today(class_id: int, today_date: date) -> dict[int, dict]:
    with get_db() as db:
        ...
        if not sess:
            return {}
        return {
            rec.student_id: {"name": rec.student.name, "reason": rec.reason}
...
def approve_request(req_id: int) -> bool:
    with get_db() as db:
        req = (
            ...
            .with_for_update()   # блокируем строку от параллельного двойного нажатия
            .first()
        )
        if not req:
            return False
...
# ── Школы (без фильтра по текущей школе — глобальные операции) ───────────────

def get_all_schools() -> list[dict]:
    with get_db() as db:
        return [{"id": s.id, "name": s.name} for s in db.query(School).all()]
...
def get_teachers_paginated(page: int = 1, per_page: int = 5) -> tuple[list[TeacherDTO], int]:
    with get_db() as db:
        query = (
            db.query(Teacher)
            .options(joinedload(Teacher.class_), joinedload(Teacher.school))
        ...
        return [_teacher_to_dto(t) for t in teachers], total


def get_students_by_class_paginated(
    class_id: int, page: int = 1, per_page: int = 8
    ...
    with get_db() as db:
        ...
        return [StudentDTO(id=s.id, name=s.name, class_id=s.class_id) for s in students], total
